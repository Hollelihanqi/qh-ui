...
import { getDefaultExportFromCjs } from './chunk-LTC2S6HL.js'
...
// ../node_modules/.pnpm/vite@5.4.10_@types+node@18.19.50_sass@1.83.4/node_modules/vite/dist/node/chunks/dep-GkhNNjoY.js
...
var base64EncodedImport = function base64EncodedConditionalImport(prelude, conditions) {
  ...
  for (const condition of conditions) {
    params = `'data:text/css;base64,${Buffer.from(`@import ${params}`).toString('base64')}' ${formatImportPrelude$1(
      condition.layer,
      condition.media,
      condition.supports,
    )}`
...
var applyConditions$1 = function applyConditions(bundle, atRule) {
  bundle.forEach((stmt) => {
    ...
    if (stmt.type === 'import') {
      stmt.node.params = base64EncodedConditionalImport2(stmt.fullUri, stmt.conditions)
      return
    }
    const { nodes } = stmt
...
function parseImport(result, atRule, conditions, from) {
  ...
  if (prev) {
    do {
      if (
        prev.type === 'comment' ||
        (prev.type === 'atrule' && (prev.name === 'charset' || (prev.name === 'layer' && !prev.nodes)))
      ) {
        prev = prev.prev()
        continue
      }
...
async function loadImportContent(result, stmt, filename, options, state, postcss) {
  ...
  const stmtDuplicateCheckKey = conditions
    .map((condition) => formatImportPrelude2(condition.layer, condition.media, condition.supports))
    .join(':')
  ...
  const importedResult = await processContent2(result, content, filename, options, postcss)
...
export { index$1 as i }
//# sourceMappingURL=dep-GkhNNjoY-7QK776XU.js.map

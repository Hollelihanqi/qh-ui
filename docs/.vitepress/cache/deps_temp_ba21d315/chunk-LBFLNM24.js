...
// ../node_modules/.pnpm/rollup@4.24.4/node_modules/rollup/dist/es/shared/node-entry.js
    ...
    if (currentNodeNeedsBoundaries || nextNodeNeedsBoundaries) {
      nextNodeStart =
        ...
        findFirstLineBreakOutsideComment(
          code.original.slice(currentNode.end, nextNode === void 0 ? end : nextNode.start),
        )[1]
...
function getCommaSeparatedNodesWithBoundaries(nodes, code, start, end) {
  ...
  for (const nextNode of nodes) {
    ...
    nextNodeStart = contentEnd =
      separator + 1 + findFirstLineBreakOutsideComment(code.original.slice(separator + 1, nextNode.start))[1]
...
function getSystemExportStatement(
  exportedVariables,
  { exportNamesByVariable, snippets: { _, getObject, getPropertyAccess } },
  modifier = '',
) {
...
function renderSystemExportExpression(
  exportedVariable,
  expressionStart,
  expressionEnd,
  code,
  { exportNamesByVariable, snippets: { _ } },
) {
...
function renderSystemExportFunction(exportedVariables, expressionStart, expressionEnd, needsParens, code, options) {
  ...
  code.prependRight(
    expressionStart,
    getDirectReturnIifeLeft(['v'], `${getSystemExportStatement(exportedVariables, options)},${_}v`, {
...
function renderSystemExportSequenceAfterExpression(
  exportedVariable,
  expressionStart,
  expressionEnd,
  needsParens,
  ...
  code.appendLeft(
    expressionEnd,
    `,${_}${getSystemExportStatement([exportedVariable], options)},${_}${exportedVariable.getName(getPropertyAccess)}`,
  )
  if (needsParens) {
...
function renderSystemExportSequenceBeforeExpression(
  exportedVariable,
  expressionStart,
  expressionEnd,
  needsParens,
...
var getPureFunctions = ({ treeshake }) => {
  ...
  for (const functionName of treeshake ? treeshake.manualPureFunctions : []) {
    ...
    for (const pathSegment of functionName.split('.')) {
      currentFunctions =
        currentFunctions[pathSegment] || (currentFunctions[pathSegment] = /* @__PURE__ */ Object.create(null))
    }
  ...
  getEntities(path) {
    ...
    for (const pathSegment of path) {
      currentPaths = currentPaths[pathSegment] =
        currentPaths[pathSegment] || Object.create(null, { [EntitiesKey]: { value: /* @__PURE__ */ new Set() } })
    }
...
var DiscriminatedPathTracker = class {
  ...
  trackEntityAtPathAndGetIfTracked(path, discriminator, entity) {
    ...
    for (const pathSegment of path) {
      currentPaths = currentPaths[pathSegment] =
        currentPaths[pathSegment] || Object.create(null, { [EntitiesKey]: { value: /* @__PURE__ */ new Map() } })
    }
...
var UNKNOWN_RETURN_EXPRESSION = [UNKNOWN_EXPRESSION, false]
...
var Variable = class extends ExpressionEntity {
  ...
  getBaseVariableName() {
    var _a
    return (
      ((_a = this.renderedLikeHoisted) == null ? void 0 : _a.getBaseVariableName()) ||
      this.renderBaseName ||
      this.renderName ||
      this.name
    )
...
var ExternalModule = class {
  ...
  warnUnusedImports() {
    const unused = [...this.declarations]
      .filter(
        ([name, declaration]) => name !== '*' && !declaration.included && !this.reexported && !declaration.referenced,
      )
      .map(([name]) => name)
...
var NodeBase = class extends ExpressionEntity {
  ...
  hasEffects(context) {
    ...
    for (const key of childNodeKeys[this.type]) {
      ...
      if (Array.isArray(value)) {
        for (const child of value) {
          if (child == null ? void 0 : child.hasEffects(context)) return true
        }
  ...
  hasEffectsAsAssignmentTarget(context, _checkAccess) {
    return (
      this.hasEffects(context) || this.hasEffectsOnInteractionAtPath(EMPTY_PATH, this.assignmentInteraction, context)
    )
  }
...
function hasMemberEffectWhenCalled(members, memberName, interaction, context) {
  ...
  return (
    ((_b = (_a = members[memberName]).hasEffectsWhenCalled) == null ? void 0 : _b.call(_a, interaction, context)) ||
    false
  )
}
  ...
  getReturnExpressionWhenCalledAtPath(path, { args }) {
    ...
    return [
      this.description.returnsPrimitive ||
        (this.description.returns === 'self' ? args[0] || UNKNOWN_EXPRESSION : this.description.returns()),
      false,
    ]
  }
  hasEffectsOnInteractionAtPath(path, interaction, context) {
    ...
    if (type === INTERACTION_CALLED) {
      ...
      if (
        ...
        ((_a = args[0]) == null
          ? void 0
          : _a.hasEffectsOnInteractionAtPath(UNKNOWN_INTEGER_PATH, NODE_INTERACTION_UNKNOWN_ASSIGNMENT, context))
      ) {
        return true
      }
      if (this.description.callsArgs) {
        for (const argumentIndex of this.description.callsArgs) {
          if (
            (_b = args[argumentIndex + 1]) == null
              ? void 0
              : _b.hasEffectsOnInteractionAtPath(EMPTY_PATH, NODE_INTERACTION_UNKNOWN_CALL, context)
          ) {
            return true
          }
        }
...
var ObjectEntity = class extends ExpressionEntity {
  ...
  deoptimizeArgumentsOnInteractionAtPath(interaction, path, recursionTracker) {
    ...
    if (
      this.hasLostTrack || // single paths that are deoptimized will not become getters or setters
      ((type === INTERACTION_CALLED || path.length > 1) &&
        (this.hasUnknownDeoptimizedProperty || (typeof key === 'string' && this.deoptimizedPaths[key])))
    ) {
      deoptimizeInteraction(interaction)
      return
    }
    ...
    ;(_a = this.prototypeExpression) == null
      ? void 0
      : _a.deoptimizeArgumentsOnInteractionAtPath(interaction, path, recursionTracker)
  }
  ...
  deoptimizePath(path) {
    ...
    ;(_a = this.prototypeExpression) == null
      ? void 0
      : _a.deoptimizePath(path.length === 1 ? [...path, UnknownKey] : path)
  }
  ...
  buildPropertyMaps(properties) {
    ...
    for (let index = properties.length - 1; index >= 0; index--) {
      ...
      if (typeof key === 'string') {
        ...
        if (kind === 'set') unmatchableSetters.push(property2)
        if (kind === 'get') unmatchableGetters.push(property2)
        if (kind !== 'get') unmatchablePropertiesAndSetters.push(property2)
        if (kind !== 'set') unmatchablePropertiesAndGetters.push(property2)
      }
    }
  }
...
var ArrayExpression = class extends NodeBase {
  ...
  getObjectEntity() {
    ...
    const properties = [{ key: 'length', kind: 'init', property: UNKNOWN_LITERAL_NUMBER }]
    let hasSpread = false
    for (let index = 0; index < this.elements.length; index++) {
      const element = this.elements[index]
  ...
  hasEffectsOnInteractionAtPath(path, interaction, context) {
    switch (interaction.type) {
      ...
      case INTERACTION_CALLED: {
        if (this.isReassigned) return true
        return (
          !(interaction.withNew ? context.instantiated : context.called).trackEntityAtPathAndGetIfTracked(
            path,
            interaction.args,
            this,
          ) && this.init.hasEffectsOnInteractionAtPath(path, interaction, context)
...
var IdentifierBase = class extends NodeBase {
  ...
  hasEffects(context) {
    ...
    return (
      this.scope.context.options.treeshake.unknownGlobalSideEffects &&
  ...
  hasEffectsOnInteractionAtPath(path, interaction, context) {
    switch (interaction.type) {
      ...
      case INTERACTION_ASSIGNED: {
        return (path.length > 0 ? this.getVariableRespectingTDZ() : this.variable).hasEffectsOnInteractionAtPath(
          path,
          interaction,
          context,
        )
  ...
  isPossibleTDZ() {
    ...
    if (
      !(
        ...
        this.variable.kind &&
        tdzVariableKinds.has(this.variable.kind) && // We ignore modules that did not receive a treeshaking pass yet as that
...
var Identifier2 = class extends IdentifierBase {
  ...
  render(
    code,
    { snippets: { getPropertyAccess }, useOriginalName },
    { renderedParentType, isCalleeOfRenderedParent, isShorthandProperty } = BLANK,
  ) {
    if (this.variable) {
...
function getSafeName(baseName, usedNames, forbiddenNames) {
  ...
  while (
    ...
    (forbiddenNames == null ? void 0 : forbiddenNames.has(safeName))
  ) {
  ...
  addDeclaration(identifier2, context, init2, kind) {
    ...
    if (kind === 'var') {
      ...
      const existingVariable =
        ((_a = this.hoistedVariables) == null ? void 0 : _a.get(name)) || this.variables.get(name)
      if (existingVariable) {
...
var ClassNode = class extends NodeBase {
  ...
  hasEffects(context) {
    ...
    if (!this.deoptimized) this.applyDeoptimizations()
    const initEffect =
      ((_a = this.superClass) == null ? void 0 : _a.hasEffects(context)) || this.body.hasEffects(context)
    ...
    return interaction.type === INTERACTION_CALLED && path.length === 0
      ? !interaction.withNew ||
          (this.classConstructor === null
            ? (_a = this.superClass) == null
              ? void 0
              : _a.hasEffectsOnInteractionAtPath(path, interaction, context)
  ...
  include(context, includeChildrenRecursively) {
    ...
    for (const decorator2 of this.decorators) decorator2.include(context, includeChildrenRecursively)
    if (this.id) {
  ...
  applyDeoptimizations() {
    ...
    for (const definition of this.body.body) {
      if (
        ...
        !(definition.static || (definition instanceof MethodDefinition && definition.kind === 'constructor'))
      ) {
...
var ParameterVariable = class extends LocalVariable {
  ...
  updateKnownValue(argument) {
    ...
    if (
      this.knownValue === argument ||
      (this.knownValue instanceof Identifier2 &&
  ...
  addDeclaration(identifier2, context, init2, kind) {
    ...
    if (kind === 'var') {
      ...
      if (existingVariable) {
        const existingKind = existingVariable.kind
        if (
          existingKind === 'parameter' && // If this is a destructured parameter, it is forbidden to redeclare
...
var BlockStatement2 = class extends NodeBase {
  ...
  initialise() {
    ...
    this.deoptimizeBody =
      firstBodyStatement instanceof ExpressionStatement2 && firstBodyStatement.directive === 'use asm'
  }
  render(code, options) {
    if (this.body.length > 0) {
...
var FunctionBase = class extends NodeBase {
  ...
  hasEffectsOnInteractionAtPath(path, interaction, context) {
    ...
    if (this.async) {
      ...
      if (
        returnExpression.hasEffectsOnInteractionAtPath(['then'], NODE_INTERACTION_UNKNOWN_CALL, context) ||
        (propertyReadSideEffects &&
          (propertyReadSideEffects === 'always' ||
  ...
  parseNode(esTreeNode) {
    ...
    const parameters = (this.params = params.map((parameter) =>
      new (context.getNodeConstructor(parameter.type))(this, scope).parseNode(parameter),
    ))
    scope.addParameterVariables(
      parameters.map((parameter) => parameter.declare('parameter', UNKNOWN_EXPRESSION)),
...
var ExportDefaultDeclaration2 = class extends NodeBase {
  ...
  render(code, options, nodeRenderOptions) {
    ...
    if (this.declaration instanceof FunctionDeclaration) {
      this.renderNamedDeclaration(
        code,
        declarationStart,
        this.declaration.id === null ? getFunctionIdInsertPosition(code.original, declarationStart) : null,
        options,
      )
  ...
  renderVariableDeclaration(
    code,
    declarationStart,
    { format, exportNamesByVariable, snippets: { cnst, getPropertyAccess } },
  ) {
...
var isDefaultAProperty = (interopType, externalLiveBindings) =>
  interopType === 'esModule' || (externalLiveBindings && (interopType === 'auto' || interopType === 'compat'))
...
var canDefaultBeTakenFromNamespace = (interopType, externalLiveBindings) =>
...
var HELPER_GENERATORS = {
  ...
  [INTEROP_NAMESPACE_COMPAT_VARIABLE](t, snippets, liveBindings, freeze, symbols, usedHelpers) {
    ...
    return (
      `function ${INTEROP_NAMESPACE_COMPAT_VARIABLE}(e)${_}{${n2}${t}if${_}(${getIsCompatNamespace(snippets)})${_}return e;${n2}` +
  ...
  [INTEROP_NAMESPACE_DEFAULT_VARIABLE](t, snippets, liveBindings, freeze, symbols) {
    ...
    return (
      `function ${INTEROP_NAMESPACE_DEFAULT_VARIABLE}(e)${_}{${n2}` +
      createNamespaceObject(t, t, snippets, liveBindings, freeze, symbols) +
      `}${n2}${n2}`
    )
  },
  [INTEROP_NAMESPACE_VARIABLE](t, snippets, liveBindings, freeze, symbols, usedHelpers) {
    ...
    return (
      `function ${INTEROP_NAMESPACE_VARIABLE}(e)${_}{${n2}${t}if${_}(e${_}&&${_}e.__esModule)${_}return e;${n2}` +
      createNamespaceObject(t, t, snippets, liveBindings, freeze, symbols) +
      `}${n2}${n2}`
    )
  },
...
var getDefaultLiveBinding = ({ _, getObject }) =>
...
var loopOverKeys = (body, allowVariableLoopVariable, { _, cnst, getFunctionIntro, s }) =>
...
var loopOverNamespaces = (body, useForEach, t, { _, cnst, getDirectReturnFunction, getFunctionIntro, n: n2 }) => {
  if (useForEach) {
    ...
    return `m.forEach(${left}e${_}&&${_}typeof e${_}!==${_}'string'${_}&&${_}!Array.isArray(e)${_}&&${_}Object.keys(e).forEach(${getFunctionIntro(
      ['k'],
      {
        isAsync: false,
        name: null,
...
var getWithToStringTag = (symbols, fragment, { _, getObject }) =>
  symbols ? `Object.defineProperty(${fragment},${_}Symbol.toStringTag,${_}${getToStringTagValue(getObject)})` : fragment
...
var Literal2 = class extends NodeBase {
  ...
  getLiteralValueAtPath(path) {
    if (
      path.length > 0 || // unknown literals can also be null but do not start with an "n"
      (this.value === null && this.scope.context.code.charCodeAt(this.start) !== 110) ||
      typeof this.value === 'bigint' || // to support shims for regular expressions
...
function getResolvablePropertyKey(memberExpression2) {
  return memberExpression2.computed
    ? getResolvableComputedPropertyKey(memberExpression2.property)
    : memberExpression2.property.name
}
  ...
  getPropertyKey() {
    if (this.propertyKey === null) {
      ...
      return (this.propertyKey =
        value === SymbolToStringTag ? value : typeof value === 'symbol' ? UnknownKey : String(value))
    }
    return this.propertyKey
  }
...
var MetaProperty = class extends NodeBase {
  ...
  include() {
    if (!this.included) {
      ...
      if (this.meta.name === IMPORT) {
        ...
        const metaProperty2 = (this.metaProperty =
          parent instanceof MemberExpression && typeof parent.propertyKey === 'string' ? parent.propertyKey : null)
  ...
  render(code, renderOptions) {
    ...
    let replacement = pluginDriver.hookFirstSync('resolveImportMeta', [metaProperty2, { chunkId, format, moduleId }])
    if (!replacement) {
      replacement =
        (_a = importMetaMechanisms[format]) == null
          ? void 0
          : _a.call(importMetaMechanisms, metaProperty2, { chunkId, snippets })
  ...
  setResolution(format, accessedGlobalsByScope, preliminaryChunkId) {
    ...
    const accessedGlobals = (
      ((_a = this.metaProperty) == null ? void 0 : _a.startsWith(FILE_PREFIX))
...
var getRelativeUrlFromDocument = (relativePath, umd2 = false) =>
  getResolveUrl(
    `'${escapeId(relativePath)}', ${umd2 ? `typeof document === 'undefined' ? location.href : ` : ''}document.currentScript && document.currentScript.tagName.toUpperCase() === 'SCRIPT' && document.currentScript.src || document.baseURI`,
  )
var getGenericImportMetaMechanism =
  (getUrl) =>
  (property2, { chunkId }) => {
...
var getUrlFromDocument = (chunkId, umd2 = false) =>
  `${umd2 ? `typeof document === 'undefined' ? location.href : ` : ''}(${DOCUMENT_CURRENT_SCRIPT} && ${DOCUMENT_CURRENT_SCRIPT}.tagName.toUpperCase() === 'SCRIPT' && ${DOCUMENT_CURRENT_SCRIPT}.src || new URL('${escapeId(chunkId)}', document.baseURI).href)`
...
var importMetaMechanisms = {
  ...
  cjs: getGenericImportMetaMechanism(
    (chunkId) =>
      `(typeof document === 'undefined' ? ${getFileUrlFromFullPath('__filename')} : ${getUrlFromDocument(chunkId)})`,
  ),
  iife: getGenericImportMetaMechanism((chunkId) => getUrlFromDocument(chunkId)),
  system: (property2, { snippets: { getPropertyAccess } }) =>
    property2 === null ? `module.meta` : `module.meta${getPropertyAccess(property2)}`,
  umd: getGenericImportMetaMechanism(
    (chunkId) =>
      `(typeof document === 'undefined' && typeof location === 'undefined' ? ${getFileUrlFromFullPath('__filename')} : ${getUrlFromDocument(chunkId, true)})`,
  ),
}
    ...
    return this.originalId &&
      (this.hasId ||
        !(
          ...
          this.originalId.variable instanceof UndefinedVariable || // this avoids a circular dependency
...
var NamespaceVariable = class extends Variable {
  ...
  deoptimizeArgumentsOnInteractionAtPath(interaction, path, recursionTracker) {
    ...
    if (path.length > 1 || (path.length === 1 && interaction.type === INTERACTION_CALLED)) {
      const key = path[0]
      if (typeof key === 'string') {
        ;(_a = this.getMemberVariables()[key]) == null
          ? void 0
          : _a.deoptimizeArgumentsOnInteractionAtPath(interaction, path.slice(1), recursionTracker)
      } else {
        deoptimizeInteraction(interaction)
      }
    }
  ...
  renderBlock(options) {
    const {
      ...
      indent: t,
      symbols,
      snippets: { _, cnst, getObject, getPropertyAccess, n: n2, s },
    } = options
    const memberVariables = this.getMemberVariables()
    const members = Object.entries(memberVariables)
      .filter(([_2, variable]) => variable.included)
      .map(([name2, variable]) => {
...
var ExternalChunk = class {
  ...
  getFileName() {
    ...
    return (this.fileName =
      ...
      (this.renormalizeRenderPath ? normalize((0, import_node_path.relative)(this.inputBase, this.id)) : this.id))
  }
  getImportAttributes(snippets) {
    return (
      this.importAttributes ||
      (this.importAttributes = formatAttributes(
        this.options.format === 'es' && this.options.externalImportAttributes && this.moduleInfo.attributes,
        snippets,
      ))
    )
  }
...
function getExportBlock$1(
  ...
  for (const { exported, local } of exports) {
    ...
    if (lhs !== rhs) {
      ...
      exportBlock +=
        exported === '__proto__'
          ? `Object.defineProperty(exports,${_}"__proto__",${_}{${n2}${t}enumerable:${_}true,${n2}${t}value:${_}${rhs}${n2}});`
          : `${lhs}${_}=${_}${rhs};`
    }
  }
  if (namedExportsMode) {
...
function getInteropBlock(
  ...
  for (const {
    ...
    if (isChunk) {
      for (const { imported, reexported } of [...(imports || []), ...(reexports || [])]) {
...
function amd(
  ...
  const deps = dependencies.map(
    (m) => `'${updateExtensionForRelativeAmdId(m.importPath, amd2.forceJsExtensionForImports)}'`,
  )
  ...
  const defineParameters =
    (completeAmdId ? `'${completeAmdId}',${_}` : ``) + (deps.length > 0 ? `[${deps.join(`,${_}`)}],${_}` : ``)
  const useStrict = strict ? `${_}'use strict';` : ''
  magicString.prepend(
    `${intro}${getInteropBlock(dependencies, interop, externalLiveBindings, freeze, symbols, accessedGlobals, t, snippets)}`,
  )
  const exportBlock = getExportBlock$1(
    exports,
    dependencies,
    namedExportsMode,
    interop,
  ...
  magicString
    ...
    .indent(t)
    .prepend(
      `${amd2.define}(${defineParameters}(${getNonArrowFunctionIntro(parameters, {
        isAsync: false,
        name: null,
      })}{${useStrict}${n2}${n2}`,
    )
...
function es(
  magicString,
  { accessedGlobals, indent: t, intro, outro, dependencies, exports, snippets },
  { externalLiveBindings, freeze, generatedCode: { symbols }, importAttributesKey },
) {
  const { n: n2 } = snippets
  const importBlock = getImportBlock(dependencies, importAttributesKey, snippets)
  if (importBlock.length > 0) intro += importBlock.join(n2) + n2 + n2
  ...
  if (exportBlock.length > 0) magicString.append(n2 + n2 + exportBlock.join(n2).trim())
  ...
  for (const { importPath, reexports, imports, name, attributes } of dependencies) {
    ...
    if (imports) {
      ...
      if (defaultImport && importedNames.length === 0) {
        ...
        importBlock.push(
          `import ${defaultImport ? `${defaultImport.local},${_}` : ''}{${_}${importedNames.map((specifier) => (specifier.imported === specifier.local ? specifier.imported : `${stringifyIdentifierIfNeeded(specifier.imported)} as ${specifier.local}`)).join(`,${_}`)}${_}}${_}from${_}${pathWithAssertion}`,
        )
      }
    }
    if (reexports) {
      ...
      if (namespaceReexports.length > 0) {
        ...
        for (const specifier of namespaceReexports) {
          importBlock.push(
            `export${_}{${_}${name === specifier.reexported ? name : `${name} as ${stringifyIdentifierIfNeeded(specifier.reexported)}`} };`,
          )
        }
      }
      if (namedReexports.length > 0) {
        importBlock.push(
          `export${_}{${_}${namedReexports.map((specifier) => (specifier.imported === specifier.reexported ? stringifyIdentifierIfNeeded(specifier.imported) : `${stringifyIdentifierIfNeeded(specifier.imported)} as ${stringifyIdentifierIfNeeded(specifier.reexported)}`)).join(`,${_}`)}${_}}${_}from${_}${pathWithAssertion}`,
        )
      }
    }
  }
...
function iife(
  ...
  if (namedExportsMode && hasExports) {
    if (extend) {
      deps.unshift(
        `this${keypath(name, getPropertyAccess)}${_}=${_}this${keypath(name, getPropertyAccess)}${_}||${_}{}`,
      )
      parameters.unshift('exports')
    } else {
      deps.unshift('{}')
  ...
  if (hasExports) {
    if (name && !(extend && namedExportsMode)) {
      wrapperIntro =
        (useVariableAssignment ? `var ${name}` : `this${keypath(name, getPropertyAccess)}`) + `${_}=${_}${wrapperIntro}`
    }
    if (isNamespaced) {
...
function system(
  magicString,
  { accessedGlobals, dependencies, exports, hasExports, indent: t, intro, snippets, outro, usesTopLevelAwait },
  { externalLiveBindings, freeze, name, generatedCode: { symbols }, strict, systemNullSetters },
) {
...
var getStarExcludesBlock = (starExcludes, t, { _, cnst, getObject, n: n2 }) => {
  if (starExcludes) {
    const fields = [...starExcludes].map((property2) => [property2, '1'])
...
var getImportBindingsBlock = (importBindings, t, { _, n: n2 }) =>
  importBindings.length > 0 ? `${n2}${t}var ${importBindings.join(`,${_}`)};` : ''
var getHoistedExportsBlock = (exports, t, snippets) =>
  getExportsBlock(
...
function getExportsBlock(exports, t, { _, n: n2 }) {
  ...
  return (
    `exports({${n2}` +
    exports.map(({ name, value }) => `${t}${stringifyObjectKeyIfNeeded(name)}:${_}${value}`).join(`,${n2}`) +
    `${n2}});${n2}${n2}`
  )
}
var getSyntheticExportsBlock = (exports, t, snippets) =>
  getExportsBlock(
    exports.filter((expt) => expt.expression).map((expt) => ({ name: expt.exported, value: expt.local })),
    t,
    snippets,
  )
var getMissingExportsBlock = (exports, t, snippets) =>
  getExportsBlock(
    exports
...
function umd(
  ...
  const completeAmdId = getCompleteAmdId(amd2, id)
  const amdParameters =
    (completeAmdId ? `'${completeAmdId}',${_}` : ``) + (amdDeps.length > 0 ? `[${amdDeps.join(`,${_}`)}],${_}` : ``)
  ...
  const cjsIntro = iifeNeedsGlobal
    ? `${t}typeof exports${_}===${_}'object'${_}&&${_}typeof module${_}!==${_}'undefined'${_}?${_}${cjsExport}${factoryVariable}(${cjsDeps.join(`,${_}`)})${_}:${n2}`
    : ''
  const wrapperIntro =
    `(${getNonArrowFunctionIntro(wrapperParameters, { isAsync: false, name: null })}{${n2}` +
    cjsIntro +
    `${t}typeof ${define}${_}===${_}'function'${_}&&${_}${define}.amd${_}?${_}${define}(${amdParameters}${factoryVariable})${_}:${n2}${t}${iifeStart}${iifeExport}${iifeEnd};${n2}})(${globalArgument}(${getNonArrowFunctionIntro(
      factoryParameters,
      {
        isAsync: false,
        name: null,
...
var ObjectPattern = class extends NodeBase {
  ...
  hasEffectsOnInteractionAtPath(_path, interaction, context) {
    for (const property2 of this.properties) {
      if (property2.hasEffectsOnInteractionAtPath(EMPTY_PATH, interaction, context)) return true
    }
    return false
  }
  markDeclarationReached() {
...
var AssignmentExpression = class extends NodeBase {
  ...
  render(code, options, { preventASI, renderedParentType, renderedSurroundingElement } = BLANK) {
    ...
    if (
      ...
      (renderedSurroundingElement === ExpressionStatement || renderedSurroundingElement === ArrowFunctionExpression)
    ) {
  ...
  include(context, includeChildrenRecursively) {
    ...
    if (!this.included) {
      ...
      checkTopLevelAwait: if (!this.scope.context.usesTopLevelAwait) {
        ...
        do {
          if (parent instanceof FunctionNode || parent instanceof ArrowFunctionExpression2) break checkTopLevelAwait
...
var BinaryExpression = class extends NodeBase {
  ...
  hasEffects(context) {
    if (
      this.operator === '+' &&
      this.parent instanceof ExpressionStatement2 &&
  ...
  if (node.arguments.length > 0) {
    if (node.arguments[node.arguments.length - 1].included) {
      ...
      if (lastIncludedIndex >= 0) {
        ...
        code.remove(
          findFirstOccurrenceOutsideComment(code.original, ',', node.arguments[lastIncludedIndex].end),
          node.end - 1,
        )
      } else {
...
var CallExpressionBase = class extends NodeBase {
  ...
  hasEffectsOnInteractionAtPath(path, interaction, context) {
    ...
    if (type === INTERACTION_CALLED) {
      ...
      (type === INTERACTION_ASSIGNED ? context.assigned : context.accessed).trackEntityAtPathAndGetIfTracked(path, this)
    ) {
      return false
    }
...
var CallExpression2 = class extends CallExpressionBase {
  ...
  hasEffectsAsChainElement(context) {
    ...
    return (
      !this.annotationPure &&
      (calleeHasEffects || this.callee.hasEffectsOnInteractionAtPath(EMPTY_PATH, this.interaction, context))
    )
  }
  include(context, includeChildrenRecursively) {
    if (!this.deoptimized) this.applyDeoptimizations()
    if (includeChildrenRecursively) {
      ...
      if (
        ...
        this.callee instanceof Identifier2 &&
...
var MultiExpression = class _MultiExpression extends ExpressionEntity {
  ...
  getReturnExpressionWhenCalledAtPath(path, interaction, recursionTracker, origin) {
    return [
      new _MultiExpression(
        this.expressions.map(
          (expression) =>
            expression.getReturnExpressionWhenCalledAtPath(path, interaction, recursionTracker, origin)[0],
        ),
      ),
      false,
    ]
...
var ConditionalExpression = class extends NodeBase {
  ...
  render(
    code,
    options,
    { isCalleeOfRenderedParent, preventASI, renderedParentType, renderedSurroundingElement } = BLANK,
  ) {
    const usedBranch = this.getUsedBranch()
    if (this.test.included) {
      ...
      const inclusionStart = findNonWhiteSpace(
        code.original,
        (this.consequent.included ? findFirstOccurrenceOutsideComment(code.original, '?', this.test.end) : colonPos) +
          1,
      )
      if (preventASI) {
...
var ForStatement = class extends NodeBase {
  ...
  hasEffects(context) {
    var _a, _b, _c
    if (
      ((_a = this.init) == null ? void 0 : _a.hasEffects(context)) ||
      ((_b = this.test) == null ? void 0 : _b.hasEffects(context)) ||
      ((_c = this.update) == null ? void 0 : _c.hasEffects(context))
    ) {
      return true
    }
    return hasLoopBodyEffects(context, this.body)
...
var IfStatement = class _IfStatement extends NodeBase {
  ...
  hasEffects(context) {
    ...
    return testValue
      ...
      : !!((_a = this.alternate) == null ? void 0 : _a.hasEffects(context))
  }
  include(context, includeChildrenRecursively) {
    this.included = true
    if (includeChildrenRecursively) {
  ...
  renderHoistedDeclarations(hoistedDeclarations, code, getPropertyAccess) {
    const hoistedVariables = [
      ...new Set(
        hoistedDeclarations.map((identifier2) => {
...
var ImportExpression = class extends NodeBase {
  ...
  getDeterministicImportedNames() {
    ...
    if (parent1 instanceof MemberExpression) {
      ...
      if (
        callExpression2.arguments.length !== 1 ||
        !(argument instanceof ArrowFunctionExpression2 || argument instanceof FunctionExpression)
      ) {
        return
      }
  ...
  getDynamicImportMechanismAndHelper(
    resolution,
    exportMode,
    { compact, dynamicImportInCjs, format, generatedCode: { arrowFunctions }, interop },
    { _, getDirectReturnFunction, getDirectReturnIifeLeft },
    pluginDriver,
  ) {
    ...
    switch (format) {
      ...
      case 'amd': {
        ...
        const resolveNamespace = helper
          ? `${resolveLeft}${resolve2}(/*#__PURE__*/${helper}(m))${resolveRight}`
          : resolve2
...
function getDeterministicObjectDestructure(objectPattern2) {
  ...
  for (const property2 of objectPattern2.properties) {
    if (property2.type === 'RestElement' || property2.computed || property2.key.type !== 'Identifier') return
...
var JSXElement = class extends JSXElementBase {
  ...
  renderClassicMode(code, options) {
    ...
    const { firstAttribute, hasAttributes, hasSpread, inObject, previousEnd } = this.renderAttributes(
      code,
      options,
  ...
  renderAutomaticMode(code, options) {
    ...
    let { firstAttribute, hasAttributes, hasSpread, inObject, keyAttribute, previousEnd } = this.renderAttributes(
      code,
      options,
...
var JSXOpeningFragment = class extends NodeBase {
  ...
  render(code, options) {
    ...
    if (mode !== 'preserve') {
      ...
      const fragment = [this.fragmentVariable.getName(getPropertyAccess, useOriginalName), ...nestedFragment].join('.')
...
var LabeledStatement = class extends NodeBase {
  ...
  render(code, options) {
    if (this.label.included) {
      ...
      code.remove(
        this.start,
        findNonWhiteSpace(code.original, findFirstOccurrenceOutsideComment(code.original, ':', this.label.end) + 1),
      )
    }
    this.body.render(code, options)
  }
    ...
    if (this.invalidAnnotations)
      for (const { start, end, type } of this.invalidAnnotations) {
        ...
        if (type === 'pure' || type === 'noSideEffects') {
          this.scope.context.log(
            LOGLEVEL_WARN,
            logInvalidAnnotation(this.scope.context.code.slice(start, end), this.scope.context.module.id, type),
            start,
          )
        }
      }
...
var PropertyDefinition = class extends NodeBase {
  ...
  hasEffects(context) {
    ...
    return (
      this.key.hasEffects(context) ||
      (this.static && !!((_a = this.value) == null ? void 0 : _a.hasEffects(context))) ||
...
var ReturnStatement2 = class extends NodeBase {
  hasEffects(context) {
    var _a
    if (!context.ignore.returnYield || ((_a = this.argument) == null ? void 0 : _a.hasEffects(context))) return true
    context.brokenFlow = true
    return false
  }
  include(context, includeChildrenRecursively) {
...
var SequenceExpression = class extends NodeBase {
  ...
  include(context, includeChildrenRecursively) {
    ...
    for (const expression of this.expressions) {
      if (
        includeChildrenRecursively ||
        (expression === lastExpression && !(this.parent instanceof ExpressionStatement2)) ||
...
var SwitchCase = class extends NodeBase {
  hasEffects(context) {
    var _a
    if ((_a = this.test) == null ? void 0 : _a.hasEffects(context)) return true
...
var SwitchStatement = class extends NodeBase {
  ...
  hasEffects(context) {
    ...
    for (const switchCase2 of this.cases) {
      if (switchCase2.hasEffects(context)) return true
  ...
  include(context, includeChildrenRecursively) {
    ...
    let onlyHasBrokenFlow = true
    let isCaseIncluded =
      includeChildrenRecursively || (this.defaultCase !== null && this.defaultCase < this.cases.length - 1)
...
var TaggedTemplateExpression = class extends CallExpressionBase {
  ...
  initialise() {
    ...
    this.interaction = {
      args: [this.tag instanceof MemberExpression && !this.tag.variable ? this.tag.object : null, ...this.args],
      type: INTERACTION_CALLED,
      withNew: false,
    }
  }
...
var TryStatement = class extends NodeBase {
  ...
  hasEffects(context) {
    var _a
    return (
      (this.scope.context.options.treeshake.tryCatchDeoptimization
        ? this.block.body.length > 0
        : this.block.hasEffects(context)) || !!((_a = this.finalizer) == null ? void 0 : _a.hasEffects(context))
    )
  }
  include(context, includeChildrenRecursively) {
    var _a, _b
    const tryCatchDeoptimization =
      (_a = this.scope.context.options.treeshake) == null ? void 0 : _a.tryCatchDeoptimization
    const { brokenFlow, includedLabels } = context
...
var UnaryExpression = class extends NodeBase {
  ...
  hasEffects(context) {
    if (!this.deoptimized) this.applyDeoptimizations()
    if (this.operator === 'typeof' && this.argument instanceof Identifier2) return false
    return (
      this.argument.hasEffects(context) ||
...
var VariableDeclaration = class extends NodeBase {
  ...
  renderDeclarationEnd(
    ...
    if (lastSeparatorPos === null) {
      ...
      if (
        ...
        (code.original.charCodeAt(this.end) === 10 || code.original.charCodeAt(this.end) === 13)
      ) {
        actualContentEnd--
  ...
  renderReplacedDeclarations(code, options) {
    ...
    for (const { node, start, separator, contentEnd, end } of separatedNodes) {
      ...
      if (
        !node.id.included ||
        (node.id instanceof Identifier2 && isReassignedExportsMember(node.id.variable, options.exportNamesByVariable))
      ) {
...
function gatherSystemExportsAndGetSingleExport(separatedNodes, options, aggregatedSystemExports) {
  ...
  if (options.format === 'system') {
    for (const { node } of separatedNodes) {
      if (
        node.id instanceof Identifier2 &&
        node.init &&
        aggregatedSystemExports.length === 0 &&
        ((_a = options.exportNamesByVariable.get(node.id.variable)) == null ? void 0 : _a.length) === 1
      ) {
...
var bufferParsers = [
  ...
  function catchClause(node, position, buffer) {
    ...
    const parameter = (node.param =
      parameterPosition === 0 ? null : convertNode2(node, scope, parameterPosition, buffer))
  ...
  function ifStatement(node, position, buffer) {
    ...
    node.alternate =
      alternatePosition === 0
        ? null
        : convertNode2(node, (node.alternateScope = new TrackingScope(scope)), alternatePosition, buffer)
  },
  ...
  function jsxElement(node, position, buffer) {
    ...
    node.closingElement =
      closingElementPosition === 0 ? null : convertNode2(node, scope, closingElementPosition, buffer)
  },
...
function getAttributesFromImportExpression(node) {
  ...
  const assertProperty =
    (_a = options.properties.find((property2) => ATTRIBUTE_KEYWORDS.has(getPropertyKey(property2)))) == null
      ? void 0
      : _a.value
  if (!assertProperty) {
    return EMPTY_OBJECT
...
function getAttributesFromImportExportDeclaration(attributes) {
  return (attributes == null ? void 0 : attributes.length)
...
function getVariableForExportNameRecursive(
  ...
  isExportAllSearch,
  searchedNamesAndModules = /* @__PURE__ */ new Map(),
) {
...
var Module = class _Module {
  ...
  getSyntheticNamespace() {
    if (this.syntheticNamespace === null) {
      ...
      ;[this.syntheticNamespace] = this.getVariableForExportName(
        typeof this.info.syntheticNamedExports === 'string' ? this.info.syntheticNamedExports : 'default',
        { onlyExplicit: true },
      )
    }
  ...
  getVariableForExportName(
    name,
    { importerForSideEffects, isExportAllSearch, onlyExplicit, searchedNamesAndModules } = EMPTY_OBJECT,
  ) {
    if (name[0] === '*') {
      if (name.length === 1) {
      ...
      const name =
        specifier instanceof ImportDefaultSpecifier
          ...
          : specifier instanceof ImportNamespaceSpecifier
            ? '*'
            : specifier.imported instanceof Identifier2
...
var copyNameToModulesMap = (searchedNamesAndModules) =>
  searchedNamesAndModules && new Map(Array.from(searchedNamesAndModules, ([name, modules]) => [name, new Set(modules)]))
var concatSeparator = (out, next) =>
  next
    ? `${out}
${next}`
    : out
var concatDblSeparator = (out, next) =>
  next
    ? `${out}
  ...
  for (const variable of imports) {
    ...
    if (variable.isNamespace && (preserveModules || module instanceof ExternalModule)) {
      variable.setRenderNames(
        null,
        (module instanceof ExternalModule ? externalChunkByModule.get(module) : chunkByModule.get(module)).variableName,
      )
...
function deconflictImportsOther(
  usedNames,
  imports,
  { deconflictedDefault, deconflictedNamespace, dependencies },
  interop,
  preserveModules,
  externalLiveBindings,
  chunkByModule,
  ...
  for (const variable of imports) {
    ...
    if (module instanceof ExternalModule) {
      ...
      if (name === 'default') {
        ...
        variable.setRenderNames(
          null,
          namespaceInteropHelpersByInteropType[interop(module.id)] ? chunk.namespaceVariableName : chunk.variableName,
        )
      } else {
...
function deconflictTopLevelVariables(usedNames, modules, includedNamespaces) {
  for (const module of modules) {
    for (const variable of module.scope.variables.values()) {
      if (
        variable.included && // this will only happen for exports in some formats
        !(
...
var REPLACER_REGEX = new RegExp(
  `${hashPlaceholderLeft}[0-9a-zA-Z_$]{1,${MAX_HASH_SIZE - hashPlaceholderOverhead}}${hashPlaceholderRight}`,
  'g',
)
var replacePlaceholders = (code, hashesByPlaceholder) =>
  code.replace(REPLACER_REGEX, (placeholder) => hashesByPlaceholder.get(placeholder) || placeholder)
var replaceSinglePlaceholder = (code, placeholder, value) =>
...
function renderNamePattern(pattern, patternName, replacements) {
  ...
  return pattern.replace(/\[(\w+)(:\d+)?]/g, (_match, type, size) => {
    if (!replacements.hasOwnProperty(type) || (size && type !== 'hash')) {
      return error(
        logFailedValidation(`"[${type}${size || ''}]" is not a valid placeholder in the "${patternName}" pattern.`),
      )
    }
...
var NON_ASSET_EXTENSIONS = /* @__PURE__ */ new Set(['.js', '.jsx', '.ts', '.tsx', '.mjs', '.mts', '.cjs', '.cts'])
  ...
  static generateFacade(
    ...
    for (const dependency of facadedModule.getDependenciesToBeIncluded()) {
      chunk.dependencies.add(
        dependency instanceof Module ? chunkByModule.get(dependency) : externalChunkByModule.get(dependency),
      )
    }
    if (
  ...
  finalizeChunk(code, map, sourcemapFileName, hashesByPlaceholder) {
    ...
    return {
      ...
      importedBindings: Object.fromEntries(
        Object.entries(renderedChunkInfo.importedBindings).map(([fileName2, bindings]) => [
          finalize(fileName2),
          bindings,
        ]),
      ),
      ...
      if (!this.facadeModule) {
        const needsStrictFacade =
          ...
          (module.preserveSignature === 'strict' ||
            (module.preserveSignature === 'exports-only' && module.getExportNamesByVariable().size > 0))
  ...
  getPreliminaryFileName() {
    ...
    if (file) {
      ...
      const [pattern, patternName] =
        preserveModules || ((_a = this.facadeModule) == null ? void 0 : _a.isUserDefinedEntryPoint)
      ...
      fileName = renderNamePattern(
        ...
        patternName,
        {
          format: () => format,
          hash: (size) =>
            hashPlaceholder || (hashPlaceholder = this.getPlaceholder(patternName, size || DEFAULT_HASH_SIZE)),
  ...
  async render() {
    ...
    if (hoistTransitiveImports && !preserveModules && facadeModule !== null) {
      for (const dep of dependencies) {
        if (dep instanceof _Chunk) this.inlineChunkDependencies(dep)
      }
    }
    const preliminaryFileName = this.getPreliminaryFileName()
    const preliminarySourcemapFileName = this.getPreliminarySourcemapFileName()
    const { accessedGlobals, indent, magicString, renderedSource, usedModules, usesTopLevelAwait } = this.renderModules(
...
`)
      ...
      if (!(variable instanceof SyntheticNamedExportVariable)) {
        ...
        if (module) {
          ...
          if (chunk !== this) {
            ...
            const importedByReexported =
              reexports == null ? void 0 : reexports.find(({ reexported }) => reexported === exportName)
            const isImported =
              imports == null
                ? void 0
                : imports.find(
                    ({ imported }) =>
                      imported === (importedByReexported == null ? void 0 : importedByReexported.imported),
                  )
            if (!isImported) {
              continue
            }
      ...
      if (variable instanceof LocalVariable) {
        for (const declaration of variable.declarations) {
          if (
            ...
            (declaration instanceof ExportDefaultDeclaration2 && declaration.declaration instanceof FunctionDeclaration)
          ) {
            hoisted = true
            break
          }
          ...
          if (addNonNamespacesAndInteropHelpers) {
            if (variable.name === 'default') {
              ...
              (this.imports.has(variable) ||
                !((_a = this.exportNamesByVariable.get(variable)) == null
                  ? void 0
                  : _a.every((name) => name.startsWith('*'))))
            ) {
  ...
  getDynamicDependencies() {
    return this.getIncludedDynamicImports()
      ...
      .filter(
        (resolution) => resolution !== this && (resolution instanceof _Chunk || resolution instanceof ExternalChunk),
      )
  }
  ...
  getPreserveModulesChunkNameFromModule(module) {
    ...
    if (isAbsolute(idWithoutExtension)) {
      ...
      return (
        this.outputOptions.virtualDirname.replace(/\/$/, '') + '/' + (0, import_node_path.basename)(idWithoutExtension)
      )
    }
  }
    ...
    for (let exportName of this.getExportNames()) {
      ...
      if (exportName[0] === '*') {
        ...
        if (module instanceof Module) {
          ...
          needsLiveBinding =
            externalLiveBindings && (imported !== 'default' || isDefaultAProperty(interop(module.id), true))
        }
      }
  ...
  renderModules(fileName) {
    ...
    for (const module of orderedModules) {
      ...
      if (module.isIncluded() || includedNamespaces.has(module)) {
        ...
        if (includedNamespaces.has(module)) {
          ...
          if (namespace.renderFirst()) hoistedSource += n2 + rendered2
          else magicString.addSource(new MagicString(rendered2))
        }
    ...
    if (hoistedSource) magicString.prepend(hoistedSource + n2 + n2)
      ...
      if (resolvedDynamicImport.chunk) {
        ...
        if (chunk === this) {
          ...
          node.setExternalResolution(
            ...
            !(facadeChunk == null ? void 0 : facadeChunk.strictFacade) &&
...
function getPredefinedChunkNameFromModule(module) {
  var _a, _b
  return (
    ((_a = module.chunkNames.find(({ isUserDefined }) => isUserDefined)) == null ? void 0 : _a.name) ??
    ((_b = module.chunkNames[0]) == null ? void 0 : _b.name)
  )
}
...
function getChunkAssignments(entries, manualChunkAliasByEntry, minChunkSize, log) {
  ...
  const { allEntries, dependentEntriesByModule, dynamicallyDependentEntriesByDynamicEntry, dynamicImportsByEntry } =
  ...
  const { chunks, sideEffectAtoms, sizeByAtom } = getChunksWithSameDependentEntriesAndCorrelatedAtoms(
    chunkAtoms,
    staticDependencyAtomsByEntry,
  ...
  chunkDefinitions.push(
    ...getOptimizedChunks(chunks, minChunkSize, sideEffectAtoms, sizeByAtom, log).map(({ modules }) => ({
      alias: null,
      modules,
    })),
  )
...
function getChunkDefinitionsFromManualChunks(manualChunkAliasByEntry) {
  ...
  for (const [entry, alias] of manualChunkAliasByEntry) {
    addStaticDependenciesToManualChunk(
      entry,
      manualChunkModulesByAlias[alias] || (manualChunkModulesByAlias[alias] = []),
...
function getChunksWithSameDependentEntries(modulesWithDependentEntries) {
  ...
  for (const { dependentEntries, modules } of modulesWithDependentEntries) {
    ...
    ;(
      chunkModules[(_a = String(chunkSignature))] ||
      (chunkModules[_a] = {
...
function getAlreadyLoadedAtomsByEntry(
  ...
  const alreadyLoadedAtomsByEntry = allEntries.map((_entry, entryIndex) =>
...
function getChunksWithSameDependentEntriesAndCorrelatedAtoms(
  ...
  for (const { dependentEntries, modules } of chunkAtoms) {
    ...
    const chunk =
      chunksBySignature[(_a = String(chunkSignature))] ||
      (chunksBySignature[_a] = {
        containedAtoms: 0n,
        correlatedAtoms,
...
function commondir(files) {
  ...
  if (files.length === 1) return (0, import_node_path.dirname)(files[0])
  const commonSegments = files.slice(1).reduce(
    (commonSegments2, file) => {
      const pathSegments = file.split(/\/+|\\+/)
      let index
      for (
        index = 0;
        commonSegments2[index] === pathSegments[index] && index < Math.min(commonSegments2.length, pathSegments.length);
        index++
      );
...
function getGenerateCodeSnippets({
  compact,
  ...
  const getNonArrowFunctionIntro = (parameters, { isAsync, name }) =>
    `${isAsync ? `async ` : ''}function${name ? ` ${name}` : ''}${_}(${parameters.join(`,${_}`)})${_}`
  ...
  const getDirectReturnFunction = (parameters, { functionReturn, lineBreakIndent, name }) => [
    ...
    arrowFunctions
      ? `${name ? ';' : ''}${lineBreakIndent ? `${n2}${lineBreakIndent.base}` : ''}`
      : `${s}${lineBreakIndent ? `${n2}${lineBreakIndent.base}` : _}}`,
  ]
...
function getCollapsedSourcemap(id, originalCode, originalSourcemap, sourcemapChain, linkMap) {
  ...
  if (originalSourcemap) {
    ...
    const baseSources = sources.map(
      (source2, index) =>
        new Source((0, import_node_path.resolve)(directory, sourceRoot, source2), sourcesContent[index]),
    )
...
function collapseSourcemaps(file, map, modules, bundleSourcemapChain, excludeContent, log) {
  ...
  const moduleSources = modules
    ...
    .map((module) =>
      getCollapsedSourcemap(module.id, module.originalCode, module.originalSourcemap, module.sourcemapChain, linkMap),
    )
...
async function transformChunk(magicString, fileName, usedModules, chunkGraph, options, outputPluginDriver, log) {
  ...
  if (sourcemap) {
    ...
    let resultingFile
    if (file) resultingFile = (0, import_node_path.resolve)(sourcemapFile || file)
    else if (dir) resultingFile = (0, import_node_path.resolve)(dir, fileName)
    else resultingFile = (0, import_node_path.resolve)(fileName)
    ...
    renderedChunks.map(
      async ({
        ...
        const transformedChunk = {
          ...
          fileName,
          sourcemapFileName:
            (preliminarySourcemapFileName == null ? void 0 : preliminarySourcemapFileName.fileName) ?? null,
        ...
        const sourcemapHashPlaceholder =
          preliminarySourcemapFileName == null ? void 0 : preliminarySourcemapFileName.hashPlaceholder
    ...
    replaceContext = (pluginContext, plugin) => ({
      ...
      resolve: (source2, importer2, { attributes: attributes2, custom, isEntry: isEntry2, skipSelf } = BLANK) => {
        ...
        return moduleLoaderResolveId(
          ...
          custom,
          isEntry2,
          attributes2 || EMPTY_OBJECT,
          skipSelf ? [...skip, { importer: importer2, plugin, source: source2 }] : skip,
        )
      },
    })
  }
...
async function resolveId(
  ...
  if (importer !== void 0 && !isAbsolute(source) && source[0] !== '.') return null
  return addJsExtensionIfNecessary(
    importer
      ? (0, import_node_path.resolve)((0, import_node_path.dirname)(importer), source)
      : (0, import_node_path.resolve)(source),
    preserveSymlinks,
  )
}
...
var getOnLog = (config, logLevel, printLog = defaultPrintLog) => {
  ...
  if (onLog) {
    ...
    return (level, log) =>
      onLog(level, addLogToString(log), (level2, handledLog) => {
...
var getDefaultOnLog = (printLog, onwarn) =>
  onwarn
    ? (level, log) => {
...
var normalizeLog = (log) =>
  typeof log === 'string' ? { message: log } : typeof log === 'function' ? normalizeLog(log()) : log
  ...
  if (typeof value === 'string') {
    ...
    error(
      logInvalidOption(
        optionName,
        urlSnippet,
        `valid values are ${additionalValues}${printQuotedStringList(Object.keys(presets))}. You can also supply an object for more fine-grained control`,
        value,
      ),
    )
  }
...
async function transform(source, module, pluginDriver, log) {
  ...
  const getLogHandler2 = (handler) => (log2, pos) => {
    ...
    if (pos) augmentCodeLocation(log2, pos, currentSource, id)
    log2.id = id
    log2.hook = 'transform'
    handler(log2)
  }
...
var ModuleLoader = class {
  constructor(graph, modulesById, options, pluginDriver) {
    ...
    this.resolveId = async (source, importer, customOptions, isEntry, attributes, skip = null) =>
  ...
  async addAdditionalModules(unresolvedModules, isAddForManualChunks) {
    const result = this.extendLoadModulesPromise(
      Promise.all(unresolvedModules.map((id) => this.loadEntryModule(id, false, void 0, null, isAddForManualChunks))),
    )
  ...
  addEntryWithImplicitDependants(unresolvedModule, implicitlyLoadedAfter) {
    ...
    return this.extendLoadModulesPromise(
      this.loadEntryModule(unresolvedModule.id, false, unresolvedModule.importer, null).then(async (entryModule) => {
  ...
  fetchResolvedDependency(source, importer, resolvedId) {
    if (resolvedId.external) {
      ...
      if (!externalModule) {
        ...
        this.options.onLog(
          LOGLEVEL_WARN,
          logInconsistentImportAttributes(externalModule.info.attributes, attributes, source, importer),
        )
      }
  ...
  getNormalizedResolvedIdWithoutDefaults(resolveIdResult, importer, source) {
    ...
    if (resolveIdResult) {
      ...
      return {
        external:
          external && (isNotAbsoluteExternal(resolveIdResult, source, makeAbsoluteExternalsRelative) || 'absolute'),
        id:
  ...
  async resolveDynamicImport(module, specifier, importer, attributes) {
    const resolution = await this.pluginDriver.hookFirst('resolveDynamicImport', [specifier, importer, { attributes }])
    ...
    if (resolution == null) {
      ...
      if (existingResolution) {
        if (doAttributesDiffer(existingResolution.attributes, attributes)) {
          this.options.onLog(
            LOGLEVEL_WARN,
            logInconsistentImportAttributes(existingResolution.attributes, attributes, specifier, importer),
          )
        }
...
function normalizeRelativeExternalId(source, importer) {
  return isRelative(source)
    ? importer
      ? (0, import_node_path.resolve)(importer, '..', source)
      : (0, import_node_path.resolve)(source)
    : source
}
function addChunkNamesToModule(module, { fileName, name }, isUserDefined, priority) {
  ...
  if (fileName !== null) {
    ...
    let namePosition = 0
    while (((_a = module.chunkNames[namePosition]) == null ? void 0 : _a.priority) < priority) namePosition++
  ...
  return makeUnique(
    renderNamePattern(
      ...
      'output.assetFileNames',
      {
        ...
        name: () =>
          emittedName.slice(0, Math.max(0, emittedName.length - (0, import_node_path.extname)(emittedName).length)),
      },
    ),
    bundle,
  )
...
function getValidSource(source, emittedFile, fileReferenceId) {
  if (!(typeof source === 'string' || source instanceof Uint8Array)) {
    ...
    return error(
      logFailedValidation(
        `Could not set source for ${typeof assetName === 'string' ? `asset "${assetName}"` : 'unnamed asset'}, asset source needs to be a string, Uint8Array or Buffer.`,
      ),
    )
  }
  return source
  ...
  constructor(graph, options, baseFileEmitter) {
    ...
    this.emitFile = (emittedFile) => {
      if (!hasValidType(emittedFile)) {
        return error(
          logFailedValidation(
            `Emitted files must be of type "asset", "chunk" or "prebuilt-chunk", received "${emittedFile && emittedFile.type}".`,
          ),
        )
      }
      ...
      if (!hasValidName(emittedFile)) {
        return error(
          logFailedValidation(
            `The "fileName" or "name" properties of emitted chunks and assets must be strings that are neither absolute nor relative paths, received "${emittedFile.fileName || emittedFile.name}".`,
          ),
        )
      }
...
function getPluginContext(plugin, pluginCache, graph, options, fileEmitter, existingPluginNames) {
  ...
  if (!pluginCache) {
    ...
    cacheInstance = createPluginCache(
      pluginCache[cacheKey] || (pluginCache[cacheKey] = /* @__PURE__ */ Object.create(null)),
    )
  } else {
  ...
  hookReduceArg0(hookName, [argument0, ...rest], reduce, replaceContext) {
    ...
    for (const plugin of this.getSortedPlugins(hookName)) {
      promise = promise.then((argument02) =>
        this.runHook(hookName, [argument02, ...rest], plugin, replaceContext).then((result) =>
          reduce.call(this.pluginContexts.get(plugin), argument02, result, plugin),
        ),
      )
    }
    return promise
...
var ALLOWED_INTEROP_TYPES = /* @__PURE__ */ new Set(['compat', 'auto', 'esModule', 'default', 'defaultOnly'])
...
var getMinifyInternalExports = (config, format, compact) =>
...
function createOutput(outputBundle) {
  return {
    output: Object.values(outputBundle)
      .filter((outputFile) => Object.keys(outputFile).length > 0)
      .sort((outputFileA, outputFileB) => getSortingFileType(outputFileA) - getSortingFileType(outputFileB)),
  }
}
var SortingFileType
...
async function writeOutputFile(outputFile, outputOptions) {
  const fileName = (0, import_node_path.resolve)(
    outputOptions.dir || (0, import_node_path.dirname)(outputOptions.file),
    outputFile.fileName,
  )
...
var createColors = ({ useColor = isColorSupported } = {}) =>
  useColor ? colors : Object.keys(colors).reduce((colors2, key) => ({ ...colors2, [key]: String }), {})
createColors()
...
var stderr = (...parameters) =>
  import_node_process.default.stderr.write(`${parameters.join('')}
`)
function handleError(error2, recover = false) {
  ...
  if (error2.stack) {
    outputLines.push(
      dim(
        (_b = error2.stack) == null
          ? void 0
          : _b.replace(
              `${nameSection}${error2.message}
`,
              '',
            ),
      ),
  ...
  if (!recover) import_node_process.default.exit(1)
}
var commandAliases = {
  c: 'config',
  d: 'dir',
...
async function mergeOptions(config, watchMode, rawCommandOptions = EMPTY_COMMAND_OPTIONS, printLog) {
  ...
  const outputOptions = await Promise.all(
    outputOptionsArray.map((singleOutputOptions) => mergeOutputOptions(singleOutputOptions, command, log)),
  )
  warnUnknownOptions(
    command,
    [
...
async function mergeOutputOptions(config, overrides, log) {
  ...
  const outputOptions = {
    ...
    generatedCode: getObjectOption(
      ...
      overrides,
      'generatedCode',
      objectifyOptionWithPresets(generatedCodePresets, 'output.generatedCode', URL_OUTPUT_GENERATEDCODE, ''),
    ),
...
async function watchInternal(configs, emitter) {
  ...
  if (watchOptionsList.length === 0) {
    return error(
      logInvalidOption('watch', URL_WATCH, 'there must be at least one config where "watch" is not set to "false"'),
    )
  }
  await loadFsEvents()
  const { Watcher } = await import('./watch-P4ZDJZFK.js')
...
rollup/dist/es/shared/node-entry.js:
  ...
  *)
*/
//# sourceMappingURL=chunk-LBFLNM24.js.map

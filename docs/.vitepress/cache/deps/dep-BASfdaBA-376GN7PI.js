import { lib } from './chunk-M2VLJFE2.js'
import { postcss_default } from './chunk-EIADF47N.js'
import { commonjsGlobal, getDefaultExportFromCjs, require_crypto } from './chunk-ZWSIKFVV.js'
import './chunk-NLSDCWQO.js'
import { require_util } from './chunk-GN57YAIH.js'
import { require_fs } from './chunk-QZAH7VNE.js'
import './chunk-RCZZDYYZ.js'
...
// ../node_modules/.pnpm/vite@5.4.10_@types+node@18.19.50_sass@1.83.4/node_modules/vite/dist/node/chunks/dep-BASfdaBA.js
...
var replaceSymbols$1 = (css, replacements) => {
  css.walk((node2) => {
    if (node2.type === 'decl' && node2.value) {
      ...
      node2.selector = replaceValueSymbols$1(node2.selector.toString(), replacements)
...
var extractICSS$2 = (css, removeRules = true, mode = 'auto') => {
  ...
  function addImports(node2, path2) {
    ...
    icssImports[unquoted] = Object.assign(icssImports[unquoted] || {}, getDeclsObject(node2))
    if (removeRules) {
      node2.remove()
    }
  }
...
var FileSystemLoader = class {
  ...
  async fetch(_newPath, relativeTo, _trace) {
    ...
    let fileRelativePath =
      fileResolvedPath || _path.default.resolve(_path.default.resolve(this.root, relativeDir), newPath)
...
function createError(node2, graph) {
  ...
  const relatedNode = related.find((relatedNode2) => graph[relatedNode2].indexOf(node2) > -1)
      ...
      return {
        Once(root3, postcss2) {
          ...
          if (importsOrder instanceof Error) {
            const importPath = importsOrder.nodes.find((importPath2) =>
            ...
            throw decl.error(
              'Failed to resolve order of composed modules ' +
                importsOrder.nodes.map((importPath2) => '`' + importPath2 + '`').join(', ') +
                '.',
              {
...
function requireBatchedHash() {
  ...
  class BatchedHash2 {
    ...
    update(data, inputEncoding) {
      ...
      if (typeof data === 'string') {
        if (
          data.length < MAX_SHORT_STRING && // base64 encoding is not valid since it may contain padding chars
...
function getHashDigest$1(buffer, algorithm, digestType, maxLength) {
  ...
  if (algorithm === 'xxhash64') {
    ...
    hash2 = new BulkUpdateDecorator(() => crypto.createHash(algorithm), algorithm)
  }
  hash2.update(buffer)
  if (
    digestType === 'base26' ||
  ...
  while (counter < length) {
    ...
    if (codePoint < 32 || codePoint > 126) {
      ...
      if (options.escapeEverything) {
        ...
        character == '\\' ||
        (!isIdentifier2 && ((character == '"' && quote == character) || (character == "'" && quote == character))) ||
        (isIdentifier2 && regexSingleEscape.test(character))
      ) {
        value = '\\' + character
      } else {
        value = character
...
tokenTypes.word =
  ...
  tokenTypes.space =
  tokenTypes.slash =
  tokenTypes.singleQuote =
  tokenTypes.semicolon =
  tokenTypes.plus =
  tokenTypes.pipe =
  tokenTypes.openSquare =
  ...
  tokenTypes.comment =
  tokenTypes.comma =
  tokenTypes.combinator =
  tokenTypes.colon =
  tokenTypes.closeSquare =
  ...
  var Parser2 = (function () {
        ...
        switch (token[_tokenize.FIELDS.TYPE]) {
          ...
          case tokens.word:
            if (
              ...
              (lastAdded === 'value' && !(spaceAfterMeaningfulToken || node2.quoteMark))
            ) {
          ...
          case tokens.str:
            ...
            var _unescapeValue = (0, _attribute2.unescapeValue)(content),
              unescaped = _unescapeValue.unescaped,
              quoteMark = _unescapeValue.quoteMark
    ...
    _proto.convertWhitespaceNodesToSpace = function convertWhitespaceNodesToSpace(nodes, requiredSpace) {
      ...
      nodes.forEach(function (n) {
        ...
        rawSpace +=
          spaceBefore + n.value + _this2.lossySpace(n.rawSpaceAfter, requiredSpace && rawSpaceBefore.length === 0)
      })
      if (rawSpace === space2) {
        rawSpace = void 0
      }
        ...
        if (nodes.length > 0) {
          ...
          if (last) {
            var _this$convertWhitespa = this.convertWhitespaceNodesToSpace(nodes),
              space2 = _this$convertWhitespa.space,
    ...
    _proto.space = function space2() {
      ...
      if (
        ...
        this.current.nodes.every(function (node2) {
    ...
    _proto.splitWord = function splitWord(namespace2, firstCallback) {
      ...
      while (
        nextToken &&
        ~[tokens.dollar, tokens.caret, tokens.equals, tokens.word].indexOf(nextToken[_tokenize.FIELDS.TYPE])
      ) {
        this.position++
    ...
    _proto._run = function _run(rule, options) {
      ...
      return new Promise(function (resolve, reject) {
        try {
          ...
          Promise.resolve(_this.func(root3))
...
function isPseudoElement(node2) {
  return (
    ...
    (node2.value.startsWith('::') ||
      node2.value.toLowerCase() === ':before' ||
      node2.value.toLowerCase() === ':after' ||
      node2.value.toLowerCase() === ':first-letter' ||
      node2.value.toLowerCase() === ':first-line')
  )
}
      ...
      case 'pseudo': {
        ...
        if (isImportExport) {
          ...
          if (isScoped) {
            ...
            if (context.inside) {
              throw new Error(`A ${node2.value} is not allowed inside of a ${context.inside}(...)`)
            }
            childContext = {
            ...
            newNodes = node2.map((childNode) => transform(childNode, childContext))
            node2 = node2.clone()
            node2.nodes = normalizeNodeArray(newNodes)
...
function localizeDeclaration(declaration, context) {
  ...
  if (isAnimation) {
    ...
    const valueNodes = valueParser(declaration.value).walk((node2) => {
      ...
      const value =
        node2.type === 'word' && !isWordAFunctionArgument(node2, stepsFunctionNode) ? node2.value.toLowerCase() : null
      let shouldParseAnimationName = false
      if (value && validIdent.test(value)) {
        if ('$' + value in animationKeywords) {
          parsedAnimationKeywords['$' + value] =
            '$' + value in parsedAnimationKeywords ? parsedAnimationKeywords['$' + value] + 1 : 0
  ...
  return {
    ...
    Once(root3, { rule }) {
      ...
      function exportScopedName(name, rawName) {
        const scopedName = generateScopedName(rawName ? rawName : name, root3.source.input.from, root3.source.input.css)
      ...
      function localizeNode2(node2) {
        switch (node2.type) {
          ...
          case 'class':
            return selectorParser.className({
              value: exportScopedName(node2.value, node2.raws && node2.raws.value ? node2.raws.value : null),
            })
          case 'id': {
        ...
        throw new Error(`${node2.type} ("${node2}") is not allowed in a :local block`)
      }
          ...
          case 'pseudo':
            if (node2.value === ':local') {
              ...
              if (
                ...
                nextNode.value === ' ' &&
                /\\[A-F0-9]{1,6}$/.test(selector3.last.value)
              ) {
        ...
        rule2.walkDecls(/composes|compose-with/i, (decl) => {
          ...
          classes.forEach((className3) => {
            ...
            if (global) {
              ...
              throw decl.error(`referenced class name "${className3}" in ${decl.prop} not found`)
            }
          })
          decl.remove()
        })
...
function getLoader(opts, plugins) {
  ...
  return typeof opts.Loader === 'function'
    ? new opts.Loader(root3, plugins, opts.resolve)
    : new _FileSystemLoader.default(root3, plugins, opts.resolve)
}
...
*/
//# sourceMappingURL=dep-BASfdaBA-376GN7PI.js.map

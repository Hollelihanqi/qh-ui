...
# 镜像配置
@yto:registry=https://npm.yto.net.cn
...
```json
{
  ...
  "sourceRoot": "packages/yto-custom",
  "projectType": "library",
  "targets": {
    "build": {
...
```bash
# 发布包
nx publish yto-custom

# 版本更新
nx version yto-custom
```
...
```json
// packages/yto-custom/package.json
{
  "name": "@yto-custom/components",
  "version": "1.0.0",
  "dependencies": {
    "@yto-custom/utils": "workspace:*", // 工作空间依赖，任意版本
    "@yto-custom/theme-chalk": "workspace:^" // 工作空间依赖，遵循语义化版本
  },
  "peerDependencies": {
    "vue": "^3.5.13" // 外部依赖声明
  },
...
```json
// packages/yto-custom/project.json
{
  "name": "yto-custom",
  "targets": {
    "build": {
      ...
      "options": {
        "command": "pnpm build",
        "outputs": [
          "dist/yto-custom", // 主构建产物
          "dist/types" // 类型文件
        ],
        "cwd": "packages/yto-custom" // 执行目录
      },
      "dependsOn": [
        "^build", // 依赖其他包的构建
        "build:theme", // 依赖主题构建
...
```json
// packages/yto-utils/project.json
{
  "name": "yto-utils",
  "targets": {
    "build": {
      ...
      "options": {
        "command": "vite build",
        "outputs": ["dist/yto-utils"],
        "cwd": "packages/yto-utils"
      }
    }
  }
}
...
```bash
# 最大化并行构建
nx run-many -t build -p yto-utils,yto-custom --parallel=3
...
# 验证缓存配置
nx build yto-custom --skip-nx-cache=false --verbose
```

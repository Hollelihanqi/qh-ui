...
export default defineComponent({
  ...
  setup(props: DescriptionsProps, { slots }: SetupContext) {
    ...
    return () => {
      return (
        <div class={`yto-descriptions text-[14px] ${props.border ? 'descriptions-border' : ''}`}>
          {props.data ? (
            <el-row gutter={!props.border ? 16 : 0} class="overflow-x-hidden" style={{ lineHeight: props.lineHeight }}>
              {props.list.map((item: any) => {
                ...
                return (
                  _show(item) && (
                    <el-col style={getItemStyles(item)} span={item.span || props.span} class="overflow-x-hidden">

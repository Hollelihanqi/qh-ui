<template>
  <div
    :id="containerId"
    class="yto-search-container search-container overflow-hidden relative w-full"
    :class="customClass"
...
const emit = defineEmits(searchContainerEmits)
...
let containerMinHeight = ref(0)

const showCollapse = ref(false)
const collapse = ref(false)

const setContainerMaxMinHeight = (maxHeight = 0, minHeight = 0) => {
  containerMaxHeight.value = maxHeight || 0
  containerMinHeight.value = minHeight || 0
}
const handleResize = (info: any, forceUpdate = false) => {
  ...
  if (props.autoLayout) setChildWidth(itemWidth, tmpChildren) // 设置子元素宽度
  nextTick(() => {
    if (props.isCollapse) dealCollapse(num, tmpChildren) // 如果开启了折叠，则处理折叠
  })
}
  ...
  Array.from(childrenNodes).forEach((child: any, idx: number) => {
    ...
    if (unref(collapse) && !isOperateEl(child)) {
      if (row < props.collapseLine) {
        ...
        // 折叠行数所在行的最后一个元素需要隐藏
        if (currentTotalLineCols >= lineNum) {
          hideEl(child)
        } else {
          showEl(child)
        }
...
const handleContainerStyle = () => {
  ...
  if (unref(containerMinHeight) === unref(containerMaxHeight)) {
    if (unref(containerMinHeight) == 0) {
      ...
      return { height: `${unref(containerMinHeight) + getElExtraHeight(getComputedStyle(containerEl))}px` }
    }
  }
  if (!containerEl) return { height: 0 }
  const height = unref(collapse)
    ? `${unref(containerMinHeight) + getElExtraHeight(getComputedStyle(containerEl))}px`
    : `${unref(containerMaxHeight) + getElExtraHeight(getComputedStyle(containerEl))}px`
  return {
